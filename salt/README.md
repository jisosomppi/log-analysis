...
The logging frontend is Kibana, which is automatically set up and started on the master. The interface can be accessed either locally on the server (http://localhost) or from other machines (http://logserver.local). Kibana requires minimal setup after the initial scripts:
